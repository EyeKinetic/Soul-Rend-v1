    ...
    try {
        ...
        const promises = categories.map(async (category) => {
            ...
            const docs = response.documents.map(doc => ({
                ...
                badge: doc.badge || "UPDATE",
                badgeClass: doc.badgeClass || "dev",
                boardColumn: doc.boardColumn || undefined
            }));
            return docs;
        });
...
const DEV_HASH = "7309a493114b2f91dd933a027d655012ed4f5c2f27cf7c6551f92d04d513904d";
    ...
    inputImgFile.addEventListener('change', function () {
        if (this.files && this.files[0]) {
            ...
            reader.onload = function (e) {
                // Populate the URL field with the Base64 data URL
...
if (publishBtn) {
    publishBtn.addEventListener('click', () => {
        ...
        if (category === "information" && customBoardCol) {
            payload.boardColumn = customBoardCol;
        }
            ...
            try {
                ...
                if (editingPostId) {
                    ...
                    if (oldCategory !== category) {
                        ...
                        if (postIndex !== -1) postsDB[postIndex] = { ...payload, id: newDoc.$id, category };
                    } else {
                        await databases.updateDocument(APPWRITE_CONFIG.databaseId, APPWRITE_CONFIG.collections[category], editingPostId, payload);
                        if (postIndex !== -1) postsDB[postIndex] = { ...postsDB[postIndex], ...payload, category };
                    }

                    editingPostId = null;
                } else {
                    const newDoc = await databases.createDocument(APPWRITE_CONFIG.databaseId, APPWRITE_CONFIG.collections[category], ID.unique(), payload);
                    const newPost = { ...payload, id: newDoc.$id, category };
                    postsDB.push(newPost);

                    // Manually reverse post DB ordering logic here if needed, or simply let renderFeeds reverse it later
                }
